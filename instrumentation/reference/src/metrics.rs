...
impl LatencyStage {
    /// Stable, bounded telemetry label for this stage.
...
struct ExecutorMetrics {
    current_admitted: AtomicU64,
    max_admitted: AtomicU64,
    admission_rejections: AtomicU64,
    queue_bound: AtomicU64,
    workers: AtomicU64,
}
...
impl Metrics {
    ...
    /// Configure immutable executor capacity gauges.
    pub fn configure_executor(&self, queue_bound: usize, workers: usize) {
        self.executor
            .queue_bound
            .store(queue_bound as u64, Ordering::Relaxed);
        self.executor
            .workers
            .store(workers as u64, Ordering::Relaxed);
    }

    /// Record the current admitted count after successful admission.
    pub fn record_admitted(&self, current: usize) {
    ...
    /// Record completion of one admitted request.
    pub fn record_completed(&self, current_after: usize) {
        self.executor
            .current_admitted
            .store(current_after as u64, Ordering::Relaxed);
    }

    /// Record explicit admission rejection at the bounded queue.
    ...
    /// Render a bounded-cardinality Prometheus surface for OTEL collection.
    pub fn prometheus_text(&self) -> String {
        let snapshot = self.snapshot();
        let mut out = String::from(
            "# HELP llm_d_sc_requests_total Classification outcomes by cache result.\n\
             # TYPE llm_d_sc_requests_total counter\n",
        );
        out.push_str(&format!(
            "llm_d_sc_requests_total{{cache=\"hit\"}} {}\nllm_d_sc_requests_total{{cache=\"miss\"}} {}\n",
            snapshot.cache_hits, snapshot.cache_misses
        ));
        out.push_str(
            "# HELP llm_d_sc_stage_latency_seconds Cumulative latency by internal stage.\n\
             # TYPE llm_d_sc_stage_latency_seconds counter\n",
        );
        for (stage, sum) in [
            (LatencyStage::Queue, snapshot.queue),
            (LatencyStage::Tokenize, snapshot.tokenize),
            (LatencyStage::Forward, snapshot.forward),
            (LatencyStage::Total, snapshot.total),
        ] {
            out.push_str(&format!(
                "llm_d_sc_stage_latency_seconds{{stage=\"{}\"}} {:.9}\n",
                stage.as_str(),
                sum.as_secs_f64()
            ));
        }
        out.push_str(
            "# HELP llm_d_sc_stage_latency_quantile_seconds Process-local latency quantiles by internal stage.\n\
             # TYPE llm_d_sc_stage_latency_quantile_seconds gauge\n\
             # HELP llm_d_sc_stage_latency_max_seconds Maximum observed latency by internal stage.\n\
             # TYPE llm_d_sc_stage_latency_max_seconds gauge\n\
             # HELP llm_d_sc_stage_observations_total Observations by internal stage.\n\
             # TYPE llm_d_sc_stage_observations_total counter\n",
        );
        out.push_str(
            "# HELP llm_d_sc_admitted_requests Current in-flight plus queued requests.\n\
             # TYPE llm_d_sc_admitted_requests gauge\n\
             # HELP llm_d_sc_max_admitted_requests Maximum observed admitted requests.\n\
             # TYPE llm_d_sc_max_admitted_requests gauge\n\
             # HELP llm_d_sc_admission_rejections_total Requests rejected at the bounded handoff.\n\
             # TYPE llm_d_sc_admission_rejections_total counter\n\
             # HELP llm_d_sc_queue_capacity Configured total admission capacity.\n\
             # TYPE llm_d_sc_queue_capacity gauge\n\
             # HELP llm_d_sc_inference_workers Configured dedicated inference workers.\n\
             # TYPE llm_d_sc_inference_workers gauge\n",
        );
        out.push_str(&format!(
            "llm_d_sc_admitted_requests {}\nllm_d_sc_max_admitted_requests {}\nllm_d_sc_admission_rejections_total {}\nllm_d_sc_queue_capacity {}\nllm_d_sc_inference_workers {}\n",
            self.executor.current_admitted.load(Ordering::Relaxed),
            self.executor.max_admitted.load(Ordering::Relaxed),
            self.executor.admission_rejections.load(Ordering::Relaxed),
            self.executor.queue_bound.load(Ordering::Relaxed),
            self.executor.workers.load(Ordering::Relaxed),
        ));
        for stage in [
            LatencyStage::Queue,
            LatencyStage::Tokenize,
            LatencyStage::Forward,
            LatencyStage::Total,
        ] {
            let p = self.stage_percentiles(stage);
            for (quantile, value) in [("0.5", p.p50), ("0.95", p.p95), ("0.99", p.p99)] {
                out.push_str(&format!(
                    "llm_d_sc_stage_latency_quantile_seconds{{stage=\"{}\",quantile=\"{}\"}} {:.9}\n",
                    stage.as_str(), quantile, value.as_secs_f64()
                ));
            }
            out.push_str(&format!(
                "llm_d_sc_stage_latency_max_seconds{{stage=\"{}\"}} {:.9}\nllm_d_sc_stage_observations_total{{stage=\"{}\"}} {}\n",
                stage.as_str(), p.max.as_secs_f64(), stage.as_str(), p.count
            ));
        }
        out
    }
...
mod tests {
    ...
    #[test]
    fn u134_prometheus_surface_is_bounded_and_contains_no_request_data() {
        let metrics = Metrics::new();
        metrics.record_cache_hit();
        metrics.record_stage(LatencyStage::Total, Duration::from_millis(12));
        let text = metrics.prometheus_text();
        assert!(text.contains("llm_d_sc_requests_total{cache=\"hit\"} 1"));
        assert!(text.contains("stage=\"total\",quantile=\"0.99\""));
    ...
    #[test]
    fn u137_executor_metrics_export_capacity_saturation_and_rejections() {
        let metrics = Metrics::new();
        metrics.configure_executor(8, 4);
        metrics.record_admitted(1);
        metrics.record_admitted(8);
        metrics.record_admission_rejection();
        metrics.record_completed(7);
        let text = metrics.prometheus_text();
        assert!(text.contains("llm_d_sc_admitted_requests 7"));
        assert!(text.contains("llm_d_sc_max_admitted_requests 8"));
        assert!(text.contains("llm_d_sc_admission_rejections_total 1"));
        assert!(text.contains("llm_d_sc_queue_capacity 8"));
        assert!(text.contains("llm_d_sc_inference_workers 4"));
    }
