...
impl<R: ClassifierRuntime + Send + Sync + 'static> InferenceExecutor<R> {
    ...
    pub fn spawn_with_workers(service: R, metrics: Metrics, bound: usize, workers: usize) -> Self {
        ...
        metrics.configure_executor(bound, workers);
                    ...
                    .spawn({
                        ...
                        move || {
                            ...
                            loop {
                                ...
                                let previous = current.fetch_sub(1, Ordering::SeqCst);
                                metrics.record_completed(previous.saturating_sub(1));
                            }
                        }
                    })
    ...
    pub fn try_enqueue(
        ...
        self.metrics.record_admitted(n);
        ...
        if self.sender.try_send(job).is_err() {
            ...
            self.metrics.record_completed(previous.saturating_sub(1));
            self.metrics.record_admission_rejection();
