...
/// Environment variable overriding total admitted inference work.
pub const QUEUE_BOUND_ENV: &str = "LLM_D_SC_QUEUE_BOUND";

/// Parse the configured admission bound without reading global process state.
///
/// Keeping this pure makes invalid production configuration testable without
/// racing other tests through environment variables. Missing configuration uses
/// the backwards-compatible default; zero and malformed values fail closed.
pub fn parse_queue_bound(raw: Option<&str>) -> io::Result<usize> {
    match raw {
        None => Ok(DEFAULT_QUEUE_BOUND),
        Some(value) => {
            let bound = value.trim().parse::<usize>().map_err(|_| {
                io::Error::new(
                    io::ErrorKind::InvalidInput,
                    format!("{QUEUE_BOUND_ENV} must be a positive integer, got '{value}'"),
                )
            })?;
            if bound == 0 {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidInput,
                    format!("{QUEUE_BOUND_ENV} must be greater than zero"),
                ));
            }
            Ok(bound)
        }
    ...
    R: crate::classify::ClassifierRuntime + Send + Sync + 'static,
{
    async fn classify(
        ...
        let tracer = opentelemetry::global::tracer("llm-d-sc.grpc");
        let mut rpc_span = tracer.start("llm_d_sc.classify");
        rpc_span.set_attribute(KeyValue::new("rpc.system", "grpc"));
        rpc_span.set_attribute(KeyValue::new("rpc.service", "classify.Classify"));
        rpc_span.set_attribute(KeyValue::new("rpc.method", "Classify"));
        ...
        rpc_span.set_attribute(KeyValue::new("llm_d_sc.signal", supported.clone()));
        for signal in &req.signals {
            if signal != &supported {
                rpc_span.set_status(Status::error("invalid_argument"));
        ...
        let result = match result {
            Err(e) => {
                rpc_span.set_status(Status::error("classification_error"));
                return Err(match e {
        ...
        rpc_span.set_attribute(KeyValue::new("rpc.grpc.status_code", 0i64));
        rpc_span.set_status(Status::Ok);
...
impl ClassifyServer {
    ...
    pub fn bind_with_classifier(
        ...
        Self::bind_with_classifier_and_bound(addr, classifier, DEFAULT_QUEUE_BOUND)
    }

    /// Bind the resident classifier with an explicit total admission bound.
    ///
    /// Production uses this seam after parsing [`QUEUE_BOUND_ENV`]. Keeping the
    /// bound explicit here prevents host-specific capacity policy from leaking
    /// into the classifier runtime and lets system tests prove shedding at a
    /// measured hardware knee.
    pub fn bind_with_classifier_and_bound(
        addr: impl AsRef<str>,
        classifier: crate::classify::CandleClassifier,
        queue_bound: usize,
    ) -> io::Result<ClassifyServer> {
        if queue_bound == 0 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "queue bound must be greater than zero",
            ));
        }
...
mod admission_config_tests {
    use super::{parse_queue_bound, DEFAULT_QUEUE_BOUND};

    #[test]
    fn u112_queue_bound_defaults_and_accepts_positive_override() {
        assert_eq!(parse_queue_bound(None).unwrap(), DEFAULT_QUEUE_BOUND);
        assert_eq!(parse_queue_bound(Some("8")).unwrap(), 8);
        assert_eq!(parse_queue_bound(Some(" 16 ")).unwrap(), 16);
    }

    #[test]
    fn u113_queue_bound_rejects_zero_and_malformed_values() {
        assert!(parse_queue_bound(Some("0")).is_err());
        assert!(parse_queue_bound(Some("eight")).is_err());
        assert!(parse_queue_bound(Some("-1")).is_err());
    }
}
